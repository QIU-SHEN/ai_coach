...
'ALTER TABLE practice_records ADD COLUMN audio_type VARCHAR(20) DEFAULT \'monologue\'',
  ...
  // Only backfill when the column is freshly created — skips on every subsequent restart.
  const scoreColumnAdded = await pool.execute(
    'ALTER TABLE debrief_records ADD COLUMN overall_score DECIMAL(4,1) DEFAULT NULL'
  ).then(() => true).catch(() => false);
  if (scoreColumnAdded) {
    await pool.execute(`
      UPDATE debrief_records
      SET overall_score = CAST(JSON_UNQUOTE(JSON_EXTRACT(analysis, '$.overallScore')) AS DECIMAL(4,1))
      WHERE debrief_mode = 'post_meeting' AND overall_score IS NULL
        AND JSON_EXTRACT(analysis, '$.overallScore') IS NOT NULL
    `).catch(() => {});
  }

  // Module 4: performance indexes for high-frequency queries (parallel — no inter-dependency)
  await Promise.all([
  ...
  ].map((sql) => pool.execute(sql).catch(() => {})));
